...
def save_as_png(image_array, save_path):
    """Converts numpy array to PIL image and saves it as PNG."""
    image = Image.fromarray(np.uint8(image_array))  # Convert numpy array to image
...
def create_dark(loader, save_path=None, severity_levels=[]):
    ...
    for _, batch in enumerate(loader):
        ...
        for severity in severity_levels:
            ...
            new_filename = f"{os.path.splitext(filename)[0]}_dark_s{severity}.png"
...
def create_fog(loader, save_path=None, severity_levels=[]):
    ...
    for _, batch in enumerate(loader):
        ...
        for severity in severity_levels:
            ...
            new_filename = f"{os.path.splitext(filename)[0]}_fog_s{severity}.png"
...
def create_frost(loader, save_path=None, severity_levels=[]):
    ...
    for _, batch in enumerate(loader):
        ...
        for severity in severity_levels:
            ...
            if not (os.path.isdir(folder_path)): os.makedirs(folder_path)
            corrupted = corrupt(image, corruption_name=corruption, severity=severity)
            new_filename = f"{os.path.splitext(filename)[0]}_frost_s{severity}.png"
...
def create_snow(loader, save_path=None, severity_levels=[]):
    ...
    for _, batch in enumerate(loader):
        ...
        for severity in severity_levels:
            ...
            if not (os.path.isdir(folder_path)): os.makedirs(folder_path)
            corrupted = corrupt(image, corruption_name=corruption, severity=severity)
            new_filename = f"{os.path.splitext(filename)[0]}_snow_s{severity}.png"
...
def create_contrast(loader, save_path=None, severity_levels=[]):
    ...
    for _, batch in enumerate(loader):
        ...
        for severity in severity_levels:
            ...
            if not (os.path.isdir(folder_path)): os.makedirs(folder_path)
            corrupted = corrupt(image, corruption_name=corruption, severity=severity)
            new_filename = f"{os.path.splitext(filename)[0]}_contrast_s{severity}.png"
...
def create_defocus_blur(loader, save_path=None, severity_levels=[]):
    ...
    for _, batch in enumerate(loader):
        ...
        for severity in severity_levels:
            ...
            if not (os.path.isdir(folder_path)): os.makedirs(folder_path)
            corrupted = corrupt(image, corruption_name=corruption, severity=severity)
            new_filename = f"{os.path.splitext(filename)[0]}_defocus_blur_s{severity}.png"
...
def create_glass_blur(loader, save_path=None, severity_levels=[]):
    ...
    for _, batch in enumerate(loader):
        ...
        for severity in severity_levels:
            ...
            if not (os.path.isdir(folder_path)): os.makedirs(folder_path)
            corrupted = corrupt(image, corruption_name=corruption, severity=severity)
            new_filename = f"{os.path.splitext(filename)[0]}_glass_blur_s{severity}.png"
...
def create_motion_blur(loader, save_path=None, severity_levels=[]):
    ...
    for _, batch in enumerate(loader):
        ...
        for severity in severity_levels:
            ...
            if not (os.path.isdir(folder_path)): os.makedirs(folder_path)
            corrupted = corrupt(image, corruption_name=corruption, severity=severity)
            new_filename = f"{os.path.splitext(filename)[0]}_motion_blur_s{severity}.png"
...
def create_zoom_blur(loader, save_path=None, severity_levels=[]):
    ...
    for _, batch in enumerate(loader):
        ...
        for severity in severity_levels:
            ...
            if not (os.path.isdir(folder_path)): os.makedirs(folder_path)
            corrupted = corrupt(image, corruption_name=corruption, severity=severity)
            new_filename = f"{os.path.splitext(filename)[0]}_zoom_blur_s{severity}.png"
...
def create_elastic(loader, save_path=None, severity_levels=[]):
    ...
    for _, batch in enumerate(loader):
        ...
        for severity in severity_levels:
            ...
            if not (os.path.isdir(folder_path)): os.makedirs(folder_path)
            corrupted = corrupt(image, corruption_name=corruption, severity=severity)
            new_filename = f"{os.path.splitext(filename)[0]}_elastic_s{severity}.png"
...
def create_color_quant(loader, save_path=None, severity_levels=[]):
    ...
    for _, batch in enumerate(loader):
        ...
        for severity in severity_levels:
            ...
            if not (os.path.isdir(folder_path)): os.makedirs(folder_path)
            corrupted = corrupt(image, corruption_name=corruption, severity=severity)
            new_filename = f"{os.path.splitext(filename)[0]}_color_quant_s{severity}.png"
...
def create_gaussian_noise(loader, save_path=None, severity_levels=[]):
    ...
    for _, batch in enumerate(loader):
        ...
        for severity in severity_levels:
            ...
            if not (os.path.isdir(folder_path)): os.makedirs(folder_path)
            corrupted = corrupt(image, corruption_name=corruption, severity=severity)
            new_filename = f"{os.path.splitext(filename)[0]}_gaussian_noise_s{severity}.png"
...
def create_impulse_noise(loader, save_path=None, severity_levels=[]):
    ...
    for _, batch in enumerate(loader):
        ...
        for severity in severity_levels:
            ...
            if not (os.path.isdir(folder_path)): os.makedirs(folder_path)
            corrupted = corrupt(image, corruption_name=corruption, severity=severity)
            new_filename = f"{os.path.splitext(filename)[0]}_impulse_noise_s{severity}.png"
...
def create_shot_noise(loader, save_path=None, severity_levels=[]):
    ...
    for _, batch in enumerate(loader):
        ...
        for severity in severity_levels:
            ...
            if not (os.path.isdir(folder_path)): os.makedirs(folder_path)
            corrupted = corrupt(image, corruption_name=corruption, severity=severity)
            new_filename = f"{os.path.splitext(filename)[0]}_shot_noise_s{severity}.png"
...
def create_iso_noise(loader, save_path=None, severity_levels=[]):
    ...
    for _, batch in enumerate(loader):
        ...
        for severity in severity_levels:
            ...
            if not (os.path.isdir(folder_path)): os.makedirs(folder_path)
            corrupted = corrupt(image, corruption_name=corruption, severity=severity)
            new_filename = f"{os.path.splitext(filename)[0]}_iso_noise_s{severity}.png"
...
def create_pixelate(loader, save_path=None, severity_levels=[]):
    ...
    for _, batch in enumerate(loader):
        ...
        for severity in severity_levels:
            ...
            if not (os.path.isdir(folder_path)): os.makedirs(folder_path)
            corrupted = corrupt(image, corruption_name=corruption, severity=severity)
            new_filename = f"{os.path.splitext(filename)[0]}_pixelate_s{severity}.png"
...
def create_jpeg(loader, save_path=None, severity_levels=[]):
    ...
    for _, batch in enumerate(loader):
        ...
        for severity in severity_levels:
            ...
            if not (os.path.isdir(folder_path)): os.makedirs(folder_path)
            corrupted = corrupt(image, corruption_name=corruption, severity=severity)
            new_filename = f"{os.path.splitext(filename)[0]}_jpeg_s{severity}.png"
